...
`live-query` owns the SQL cell. `profile-source` owns the leaf schema. `promote-dataset` owns the name. A chart on that cell goes through `notebook_ggsql_check` before `notebook_run_cell`.
